...
async function handleOrderProcess() {
    ...
    try {
        ...
        // After order creation, create order details with the returned order ID
        await createOrderDetails(createdOrder.id, formDataArry);
...
async function createOrderDetails(orderId, formDataArry) {
    let orderDetails = {
        orderId: orderId,
        deliveryAdress: "123 Street",  // Replace with actual data
    ...
    try {
        const detailResponse = await fetch("/orderdetail", {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
            },
        ...
        // Clear cart and localStorage after successful order and detail creation
